...
One of the primary goals of this application \todo{is} to give the lecturer a way to specify what constitutes a correct problem solution.
As previously mentioned in the \textbf{Use Cases} section of chapter \ref{chap:Specification}, a common approach to this is the use of test cases. \todo{is this mentioned in specification?}
...
However, we want to eventually display the test code to the user so they may gain a better understanding of the solution requirements, although this will not be done in this iteration of the project. \todo{We have argued for no reason why we would need this in the solution, so maybe don't mention it. }
\todo{remove whitespace}
...
\begin{figure}[H]
	...
	\centering
	\caption{An example of a successful exercise submission \todo{not ref'ed}}
...
\begin{figure}[H]
	...
	\centering
	\caption{An example of an unsuccessful exercise submission \todo{not ref'ed}}
	\label{fig:exercise_fail}
\end{figure}
\todo{remove whitespace}
More images of the platform user interface can be seen in chapter \ref{chap:images} in the appendix. \todo{appendix not chapter.}


\section{Test Runner Process} \label{sec:test_runner_process}\todo{describtive title maybe Implementing the test runner}
The Test Runner exposes an endpoint \texttt{/haskell/submit/}, which is used to post code submissions. \todo{we never use this information for anything. Remove where the endpoint is hosted, it does not matter.}
...
We give an overview and a more technical explanation of how this was done in section \ref{sec:queue-system}.\todo{I cannot see that the above is relevant if we do not want to write about that implementation. Just write that the test runner exposes an endpoint available through http. The http server hosting the test runner is developed using the axum crate, which enables easily utilization of asynchronous programming and multithreading.}
\todo{remove whitespace}
