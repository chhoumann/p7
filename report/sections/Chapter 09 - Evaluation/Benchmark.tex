...
To achieve a varying degree of complexity for the submission content, several Haskell exercise solutions covering different concepts from course were selected, and Hspec tests based on the description of these problems were developed. \todo{create and ref to appendix containing the code and test} 
...
\begin{lstlisting}[language=CSharp, escapechar=~, caption={C\# code showing xxxxxx}, label={lst:TaskBuilder}]
...
\begin{lstlisting}[language=CSharp, escapechar=~, caption={C\# code showing xxxxxx}, label={lst:baselineBench}]
// Attributes excluded
...
\begin{lstlisting}[language=CSharp, escapechar=~, caption={C\# code showing xxxxxx}, label={lst:queueSystemBench}]
// Attributes excluded
...
\subsection{Results}
\todo{give characterization of the benchmarks, IterationCount=40	LaunchCount=1, WarmupCount=5, AMD EPYC Processor (with IBPB), 4 CPU, 4 logical and 4 physical cores, UBUNTU 20.04 LTS}
