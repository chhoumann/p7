...
The database and test runner run as processes contained in different Docker containers. The frontend and backend components run in the same container.
...
Once a user account is created, it is assigned the role either of lecturer or student.  
Depending on the account role, the user has access to different functionality.
A lecturer is able to create and modify syllabi, problem sets, submissions, problems, and tests. 
A student is able to fetch syllabi, problem sets, problems, and tests, but is not able to create or modify them. 
A student is able to create submissions through tRPC.
...
The component is implemented in TypeScript and Next.
...
A user's role is stored in the form of an encrypted cookie, which is sent to the backend whenever mutations or queries are made. 
