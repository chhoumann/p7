\chapter{Frontend} \label{chap:Frontend} \todo{I have some problems regarding this chapter. It does not describe the frontend, but rather the flow of the whole platform. This chapter can be used to highlight how we solve usecases, but at the same time it talks about almost everything. It gets very confusing when it all jumps around so much. It could maybe be named something like "Platform functionality". It could describe important functionality about creating submissions, problems etc. So start out with  {Syllabi, Problem sets and Exercises} then move to {Problem View and Code Editor} and end with user auth. Right now it focuses on nothing and therefore describes nothing meaningful - at least its hard to see what is important. }
One of the primary goals of this application to give the lecturer a way to specify what constitutes a correct problem solution. \todo{suggestion: As mentioned in section xx, a lecturer can define test in Hspec, to establish whether a function produces certain outputs given some inputs. }
As previously mentioned in the \textbf{Use Cases} section of chapter \ref{chap:Specification}, a common approach to this is the use of test cases. \todo{specification does not mention this at all}
\todo{write what this chapter describes}
...
The actions that a user is allowed to take on a page depend on their role. For example, a lecturer may have the ability to create, edit, and delete syllabi, problem sets, and exercises, whereas a student may only be able to view the information on the page. \todo{this should be under authentication maybe. So we describe how roles are assigned.}
...
\todo{ref to the figures}

\section{Problem View and Code Editor}
When a user visits a page on our website, we fetch the code template exercise on the server in order to prevent flashing in the template after the page has loaded. \todo{flashing in does not sound good, and I am unsure what it means.}
This is important because, without this, users with slow internet connections might see the template being overwritten after they have already started writing in it. That is, if you don't disable the input field entirely until the page has finished loading data. Both these approaches could lead to confusion and frustration for the user, so we decided to fetch the template before the page is rendered. \todo{what are the two approaches. I see only one. Maybe remove sentence starting with That is,}
...
Currently, only Haskell is supported, but it is easiy extensible to include other languages. \todo{this is not important. Erase that only haskell is supported. According to all we have written we do not need more languages.}
