...
The client will eagerly request new data to stay up to date with instructions or test code updates. \todo{what does eagerly mean in this case? Do we just fetch regularly? Do we just fetch related data whenever we fetch something? How does that work? If we fetch problem what do we also fetch.}
We deemed that this short cache storage time is useful for when lecturers update test code during an exercise session \todo {in case correctiont need to be made}.
This way, all students have the latest test code shortly after the update while still minimizing unnecessary queries. \todo{database or http quiries. We need to differentiate}

When a user submits their exercise solution attempt, a mutation is sent to the Next.js backend. \todo{mutation is not a term every programmer knows. Maybe if you are into frontend, but not if you are not. Either we explain what it means or we should write in common terms what it does.}
In this request, the necessary code and test code is included. \todo{haskell code and hspec test is included.}
...
More images of the platform user interface can be seen in chapter \ref{chap:images} in the appendix. \todo{can be seen in appendix \ref{chap:images} }
