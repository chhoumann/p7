...
While this approach solved the initial of only being able to process one code submission at a time, the astute reader may have considered that the hashmap continuously grows over time, never releasing the memory contained within.
Eventually, the memory use of the web server would exceed its capacity, resulting in a crash.
