...
While this approach solved the initial problem of only being able to process one code submission at a time, the astute reader may have considered the fact that the hashmap continuously grows over time --- data is only ever added to the hashmap and never removed.
Consequently, the memory use of the web server would eventually exceed its capacity, resulting in a crash.
In the following chapter, we introduce a sweeping system as a solution to this problem.