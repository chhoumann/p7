...
\section*{Glasgow Haskell Compiler} \todo{Move down to first mention of GHC (by request of carlos)}
...
A test is an assertion that a piece of code with given inputs produces the correct outputs.  \todo{ made this readable}
We can use this \todo{the framework} to automatically run the tests on the coding exercises the students submit. This allows us to confirm if the students completed the exercise and if not, provide them with feedback on which tests their code failed.
...
This makes the framework easier to work with. As a DSL is a language targeted towards a specific domain, it allows Hspec tests to be easily written and understandable without having an in-depth understanding of Hspec and Haskell. \todo{meget gentagelse}
...
An example setup of an Hspec test can be seen below in code snippet \ref{lst:HspecTestExample}. First, Hspec and the module defined in the student's code are imported. Then the specification describing the test \todo{removed "Hspec should run in the code"} is defined. In the figure \todo{ref to figure}, the function is called on the input variable \texttt{x} and the expected output is \texttt{y}, which can be a value of any type.
...
The idea behind Rust was to challenge the idea that high-level ergonomics and low-level control come at the cost of each other\cite{Rust_Book}. \todo{this is irrevelevant. remove it}
...
In order to achieve this goal Rust provides a large number of safety features among these are its most distinguishing feature which prevents invalid data access at compile time. \todo{what is invalid data}
Considering a research article by Microsoft Security Research Center stating that invalid data access is behind 70\% of serious security bugs in modern programs this is a large deal\cite{Safe_Systems_Languages}. \todo{use this source to descr. why rust is important. Move it up to before "Here Rust provides the low-level..."}
...
\begin{enumerate}
    \item Rust is compiled to native machine code, which ensures that no additional overhead is introduced by the language itself. \todo{why is this useful to us? We have at no point said that we need to be fast as fuck boy. What kind of overhead is avoided?}
...
This is described in more detail in chapter \ref{chap:TestRunner}. \todo{we dont care that we use something first and then change. We use axum.}
...
The containers themselves are created from an image. Here \todo{comma} the image is the executable package that encapsulates everything required in order to run the application. On runtime, the image is then turned into a container.
...
In tRPC, a query is like \todo{describe} a read operation, while a mutation is similar to create, update, or delete operations \todo{is similar to? How similar? Where do we know these operations from?}.
...
\todo{The main goal of Prisma is to allow the use of a type safe API for generating database queries. The results of these queries are then mapped to JS objects. This allows for the development of type-safe database access applications.}
...
Prisma enables this productivity improvement by allowing developers to define their applications models through a data modeling language granting an easier way of configuring their applications' data sources and models\cite{Prisma_Doc}. \todo{do we care about these generations? Would it not be better to write that it also allows high control while advancing productivity of developers by.....}
...
\todo{we do not say that it provides types, only that it is a superset of js.}

\section*{Next.js}
Next.js (Next) is an open-source React framework created by Vercel that enables react web applications to use server-side rendering as well as generate static websites. The Next framework was created in order to counter the potential security issues of React, which relies on client-side rendering. Next provides a solution to this by enabling websites to be fully or partially server-side rendered\cite {Nextjs_Docks}. \todo{what is react}
\todo{remove whitespace}
Next as a framework also tries to fix another issue of React, namely that creating a website from the ground up still requires a lot of effort, such as spending time on configuring tools as well as rebuilding solutions. \todo{remove as "a framework"}
...
\todo{describe mutations and how next handle them. This would be so very nice when reading the ui deep dive}
...
\end{quote}
\todo{remove whitespace    Where should be "Here" eller "in above"}
...
Before the test, the interviewers have to ensure a test plan, tasks, and questions are ready, and that the equipment works. \todo{replace the equipment with needed equipment}
...
Afterwards, the interviewers analyze the data and create a list of problems from it. \todo{we need to write that usability testing can be done using an interview.}
\cite{deb7}
...
\todo{remove whitespace, we do not say what we use it for or that it is scored on a scale}
...
The expected capabilities are often described in an operational profile\cite{OperationalProfiles}. \todo{create one for the exam to show, maybe a quick one to yeet into appendix}
