...
BenchmarkDotNet\cite{Benchmarkdotnet} is a benchmarking framework for measuring the speed of operations performed in .NET runtimes. 
...
We use this framework to create a stress test environment simulating stressful client behavior.
This way, we can measure the response times for the system under test.

We want to use results from the stress tests and benchmarks to determine whether the built-in capabilities of the HTTP server library Rocket suits the use case of the system or if the platform benefits from a multithreaded approach.
