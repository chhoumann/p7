...
Out of the currently available Haskell compilers we use the Glasgow Haskell Compiler (GHC) on the backend\todo{no mention of architecture in prelim. } for interpreting the Haskell exercises \todo{What exercises? We need to establish that we want students to be able to submit execercises before we can make this claim.} and running the test suite \todo{What is a test suit? We use 'the' - I don't know what it refers to.}. The GHC compiler recommended by the Haskell Organisation and is recommended as the go-to compiler/interpreter by the Programming Paradigms teacher.
...
The System Usability Scale (SUS) is a standardized set of usability questions, that can be used to compare a UI to UI's from other systems. It is important to note that the SUS does not provide insight into specific problems, it instead it provides an overall rating of the system from the users point of view. Other questions can be added about specific issues that the tester want addressed, but it will not be accounted for in the SUS score.

The questions that are used to determine the SUS score can be seen below
% add the questions
\begin{enumerate}
...
\end{enumerate}
% strongly agree/disagree points


If a system achieves a SUS score above 68 it means that it the UI is above average, if it is below 68, it is below average.
\cite{adobeSUS}.

% every second is negative
% how to calc
